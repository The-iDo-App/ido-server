...
exports.createUser = async(req, res) => {
    ...
    if (existingUser) {
        try {
            existingInterest = await Interest.findOne({ userId: existingUser._id });
        } catch (err) {
            throw err;
        }
        if (existingInterest) {
            let accessToken = generateToken(existingUser);
            return res.json({ user_id: existingUser._id, interestId: existingInterest._id, access_token: accessToken });
        } else {
        ...
        if (access_token) return res.json({ user_id: userId, interestId, access_token });
        return res.json({ error: "Server error, incomplete request" })
    }
};
